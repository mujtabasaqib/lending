...
pub fn process_withdraw(ctx: Context<Withdraw>, amount: u64) -> Result<()> {
  //transfer from bank token account to user token account / withdrawing
  ...
  */
  ...
  let transfer_cpi_checked = TransferChecked {
  ...
  let cpi_ctx = CpiContext::new(cpi_program, transfer_cpi_checked)
  ...
  //update user account and bank account
  let bank = &mut ctx.accounts.bank;
